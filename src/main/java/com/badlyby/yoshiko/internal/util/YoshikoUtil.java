package com.badlyby.yoshiko.internal.util;
...
import com.badlyby.yoshiko.internal.CyActivator;
import com.badlyby.yoshiko.internal.model.YoshikoAlgorithm;
import com.badlyby.yoshiko.internal.model.YoshikoCluster;
import com.badlyby.yoshiko.internal.model.YoshikoCurrentParameters;
import com.badlyby.yoshiko.internal.model.YoshikoGraph;
import com.badlyby.yoshiko.internal.util.layout.SpringEmbeddedLayouter;
import com.badlyby.yoshiko.internal.view.YoshikoLoader;
import com.badlyby.yoshiko.internal.view.YoshikoMainPanel;
...
public class YoshikoUtil {
	...
	private YoshikoCurrentParameters currentParameters;
	// Keeps track of networks (id is key) and their respective algorithms
	private Map<Long, YoshikoAlgorithm> networkAlgorithms;
	...
	private static final Logger logger = LoggerFactory.getLogger(YoshikoUtil.class);
	
	public YoshikoUtil(final RenderingEngineFactory<CyNetwork> renderingEngineFactory,
		...
		this.props = loadProperties("/Yoshiko.properties");
	...
	public void reset() {
		currentResultId = 1;
		currentParameters = new YoshikoCurrentParameters();
		networkAlgorithms = new HashMap<Long, YoshikoAlgorithm>();
		networkResults = new HashMap<Long, Set<Integer>>();
		createdSubNetworks = new HashMap<CyRootNetwork, Set<CySubNetwork>>();
	}	
	
	public synchronized void destroyUnusedNetworks(final CyNetwork network, final List<YoshikoCluster> clusters) {
	...
	public YoshikoCurrentParameters getCurrentParameters() {
	...
	public YoshikoAlgorithm getNetworkAlgorithm(final long suid) {
		return networkAlgorithms.get(suid);
	}

	public void addNetworkAlgorithm(final long suid, final YoshikoAlgorithm alg) {
	 ...
	 */
	public Image createClusterImage(final YoshikoCluster cluster,
									final int height,
									final int width,
									SpringEmbeddedLayouter layouter,
									boolean layoutNecessary,
									final YoshikoLoader loader) {
	...
	public YoshikoGraph createGraph(final CyNetwork net, final Collection<CyNode> nodes) {
	...
	public VisualStyle getClusterStyle() {
		if (clusterStyle == null) {
			clusterStyle = visualStyleFactory.createVisualStyle("Yoshiko Cluster");
	...
	public VisualStyle getAppStyle(double maxScore) {
		if (appStyle == null) {
			appStyle = visualStyleFactory.createVisualStyle("Yoshiko");

			// Node Shape:
			DiscreteMapping<String, NodeShape> nodeShapeDm = (DiscreteMapping<String, NodeShape>) discreteMappingFactory
					.createVisualMappingFunction("Yoshiko_Node_Status", String.class, NODE_SHAPE);
		...
		ContinuousMapping<Double, Paint> nodeColorCm = (ContinuousMapping<Double, Paint>) continuousMappingFactory
				.createVisualMappingFunction("Yoshiko_Score", Double.class, NODE_FILL_COLOR);
	 ...
	 */
	public boolean exportMCODEResults(final YoshikoAlgorithm alg, final List<YoshikoCluster> clusters, CyNetwork network) {
		...
		try {
			...
			if (file != null) {
				...
				// Write header
				fout.write("Yoshiko App Results" + lineSep);
	 ...
	 */
	public YoshikoMainPanel getMainPanel() {
	 ...
	 */
	public Collection<YoshikoResultsPanel> getResultPanels() {
	...
	public YoshikoResultsPanel getResultPanel(final int resultId) {
